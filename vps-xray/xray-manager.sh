...
xray_gen_link() {
    ...
    echo "vless://${UUID}@${SNI}:${PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${SNI}&fp=chrome&pbk=${PK}&sid=${SID}&type=tcp&headerType=none#${NAME}"
}
...
sub_get_url() {
    local subname=$1
    echo "https://$(xray_get_sni)/sub/${SUB_TOKEN}/$subname"
}
...
sub_create() {
    ...
    ask_input "Enter subscription name" SUBNAME || {
        pause
        return
    }
    mkdir -p "$SUB_ROOT"
    local subfile="$SUB_ROOT/$SUBNAME"
    echo -n "" >"$subfile"
...
sub_edit_file() {
    print_section "2.9" "Subscriptions › Edit subscription file"
    local subfiles=() subnames=() i=1
    for subfile in $(list_sub_files); do
        subfiles+=("$subfile")
        subnames+=("$(sub_get_name $subfile)")
    ...
    echo -e "  ${YELLOW}0.${NC} Cancel"
    echo ""
    read -p "  Select subscription to edit: " choice
    [ "$choice" = "0" ] && return
    local idx=$((choice - 1))
    local target="${subfiles[$idx]}"
    local subname="${subnames[$idx]}"
    ...
    echo -e "\n  ${YELLOW}Path:${NC} $target"
    echo -e "  ${YELLOW}URL:${NC}  $(sub_get_url $subname)"
    echo -e "  ${YELLOW}Opening decoded content in nvim...${NC}"
    sleep 1
    nvim "$tmpfile"
...
menu_xray_users() {
    while true; do
        ...
        echo -e "  ${GREEN}9.${NC} Edit subscription file"
        echo -e "  ${YELLOW}0.${NC} Back"
        echo ""
        read -p "  Choice: " c
        case $c in
...
while true; do
    print_header
    echo -e "  ${CYAN}1.${NC} Core Management"
    echo -e "  ${CYAN}2.${NC} User & Subscription Management"
    echo -e "  ${RED}3.${NC} Autodeploy"
